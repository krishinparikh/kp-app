#!/usr/bin/env node
/**
 * Enforces the design-token layering described in src/tokens/README.md:
 *
 *   primitives.css  raw values, referencing nothing
 *   semantics.css   references primitives, and only primitives
 *   theme.css       references semantics — every namespace, not just color
 ...
 *
 * The token files always come from this package. The directories to check for
 * violations are given as arguments, resolved against the caller's cwd, so each
 * app lints its own source with the same rules:
 *
 *   lint-tokens src            # from an app, via the package's bin
 *   node scripts/lint-tokens.ts src
 *
 * Runs as part of every `pnpm lint`.
 */
...
if (targets.length === 0) {
  console.error('Usage: lint-tokens <dir> [...more]')
  process.exit(1)
}
for (const dir of targets) {
  if (!fs.existsSync(dir)) {
    console.error(`lint-tokens: no such directory: ${dir}`)
    process.exit(1)
  }
}
...
const primitivesFile = path.join(tokensDir, 'primitives.css')
const semanticsFile = path.join(tokensDir, 'semantics.css')
const themeFile = path.join(tokensDir, 'theme.css')
...
// a literal here would hand components a way around the semantic layer.
...
// The one exception, and CSS forces it: these land inside `@media` and
// `@container` conditions, where `var()` is illegal. Tailwind copies the value
// in verbatim, so an indirected breakpoint emits
// `@media (width >= var(--x))` — invalid, and dropped whole by the browser —
// while an indirected container emits no rule at all. Both fail silently, so
// they are required to hold literals instead.
const QUERY_NAMESPACES = /^--(breakpoint|container)-/
for (const { name, value, line } of theme) {
  if (name === '--*') continue
  if (KEYWORDS.has(value)) continue

  if (QUERY_NAMESPACES.test(name)) {
    if (value.includes('var(')) {
      report(
        themeFile,
        line,
        `${name} references a token. Query conditions can't read a variable — keep this a literal.`,
      )
    }
    continue
  }
...
function deletedClasses(): Map<string, string> {
  // Resolved rather than joined: under pnpm, tailwindcss lives in the store and
  // is only symlinked into whichever package declared it.
  let defaultsFile: string
  try {
    defaultsFile = fileURLToPath(import.meta.resolve('tailwindcss/theme.css'))
  } catch {
    return new Map()
  }
...
// ---------------------------------------------------------------- components
// Anything outside src/tokens/ speaks in class names. Referencing a primitive,
...
for (const file of targets.flatMap((dir) => [...sourceFiles(dir)])) {
  fs.readFileSync(file, 'utf8')
    .split('\n')
    .forEach((text, index) => {
      const line = index + 1
...
console.error(
  `\n${problems.length} token layering violation(s). See ${path.relative(process.cwd(), path.join(tokensDir, 'README.md'))}.`,
)
process.exit(1)
