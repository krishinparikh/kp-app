 ...
 * barrel plus story and test stubs, and rewrites the cross-component imports
 * shadcn generates so they point at the new folders.
 */
...
const primitivesDir = path.join(webRoot, 'src/components/primitives')
...
// shadcn components import each other as '@/components/primitives/<kebab>';
// after the move the folder name is PascalCase.
for (const file of walk(primitivesDir)) {
  const before = fs.readFileSync(file, 'utf8')
  let after = before
  for (const [kebab, name] of renames) {
    after = after.replaceAll(
      `@/components/primitives/${kebab}`,
